...
for ($i = 0; $i < 20; $i++) {
	for ($j = 0; $j < 20; $j++) {
		...
		for ($k = 0; $k < 4; $k++) {
			...
			if ($j + 3 < 20) {
				$num = $numbers[$i][$j + $k];
				$horz = $k > 0 ? $horz * $num : $num;
				$horzCombo .= $num . ' ';
			}
			
			if ($i + 3 < 20) {
				$num = $numbers[$i + $k][$j];
				$vert = $k > 0 ? $vert * $num : $num;
				$vertCombo .= $num . ' ';
			}
			
			if ($i + 3 < 20 && $j + 3 < 20) {
				$num = $numbers[$i + $k][$j + $k];
				$diag = $k > 0 ? $diag * $num : $num;
				$diagCombo .= $num . ' ';
			}
			
			if ($i + 3 < 20 && $j - 3 > -1) {
				$num = $numbers[$i + $k][$j - $k];
				$saber = $k > 0 ? $saber * $num : $num;
				$saberCombo .= $num . ' ';
			}
			
		}
		
		echo  '(', $i, ', ', $j, ') [ ', $horzCombo, '], [ ' . $vertCombo, '] [ ' . $diagCombo, '] [ ' . $saberCombo, ']', PHP_EOL;
		
		if ($vert == 1492992) {
			echo $diagCombo;
		}
		
		$max = $max < $horz ? $horz : $max;
		$combo = $max == $horz ? $horzCombo : $combo;
		$max = $max < $vert ? $vert : $max;
		$combo = $max == $vert ? $vertCombo : $combo;
		$max = $max < $diag ? $diag : $max;
		$max = $max < $saber ? $saber : $max;
		$combo = $max == $diag ? $diagCombo : $combo;
